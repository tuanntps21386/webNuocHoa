...
app.controller("shopping-cart-ctrl", function($scope, $http) {
	 // Thêm biến để kiểm tra lỗi địa chỉ
    ...
    $scope.order = {
        ...
        purchase() {
            ...
            if (!$scope.order.address.trim()) {
				alert("Xin hãy điền địa chỉ nhận hàng trước khi thanh toán!");
     ...
     // Thêm hàm để ẩn thông báo lỗi khi địa chỉ được nhập
    $scope.hideAddressError = function () {
        $scope.addressError = false;
    };
    
    $http.post("/updateCount", { amount: $scope.cart.amount }).then(resp => {
